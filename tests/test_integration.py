...
def test_live_discovery():
    stations = xndbc.stations.search(
    ...
    available = xndbc.stations.availability("44013", years=2020)
    assert available.available.item()


def test_live_historical_and_realtime():
    historical = xndbc.fetch_historical("44013", 2020, progress=False)
    assert historical.sizes["time"] > 366
    assert historical.WTMP.attrs["units"] == "degC"
    realtime = xndbc.fetch_realtime("44013", progress=False)
...
def test_live_advanced_modes(mode, station, year, dimension):
    data = xndbc.fetch_historical(station, year, mode=mode, progress=False)
    assert data.sizes[dimension] > 1


def test_live_realtime_spectra():
    data = xndbc.fetch_realtime("44013", mode="swden", progress=False)
